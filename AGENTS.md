...
3. **能否不写 CoT 就追踪 OS 状态是架构决定的。** 常数深度 Transformer 与 Mamba 类 SSM 都卡在 TC⁰，[明确无法 evaluate code / track entities](https://arxiv.org/abs/2404.08819)；对角+低秩 LRNN（Gated DeltaNet 家族，即 Qwen3.5-35B-A3B 的混合层）是 [PNC¹-complete](https://arxiv.org/abs/2603.03612)，前提是特征值覆盖负值。[2602.14814](https://arxiv.org/abs/2602.14814) 的 REPL 协议可直接改造成 shell 版判定实验。
