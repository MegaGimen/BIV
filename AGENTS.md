...
**空间换时间：两个开关，逻辑独立，均默认开。**

实测 192.92 s/step（365 小时/epoch），原始（没有反坍缩损失）约 158 s/step（300 小时/epoch）。两个优化解决的是两个不同的根本开销：

1. **Selective gradient checkpointing（`selective_checkpointing: true`）。** `Qwen3_5MoeDecoderLayer` 继承 `GradientCheckpointingLayer`，每层独立持有一个 `gradient_checkpointing` 实例属性。全局 `gradient_checkpointing_enable()` 把它全部设成 `True`；之后我们逐层把**线性注意力层**（Gated DeltaNet，共 30 层，O(L) 激活显存）设回 `False`，**全注意力层**（10 层，O(L²) KV 显存——在 `max_length=65536` 时每层约 8 GiB）保持 `True`。这样 30 层的反向传播从「重算前向」变成「从存好的激活读」，省去 30 次重算前向。静态估算：30 层线性注意力在 max_length 下存激活约 25 GiB（2048 维 × 65536 × 1.8 倍系数 × 30 层 × 2 字节），落在 33 GiB 富余里。预计省约 20-25% 时间。

2. **z/z1 批合并（`batch_o_z1: true`）。** 目前每个 micro-batch 要对 `o_ids` 做两次独立前向：一次 `no_grad` 算 z（给 `loss_align`/`loss_inv`），一次活前向算 z1（给 `loss_simcse`）。这两次用的是同一份 `o_ids`，唯一差异是 dropout 掩码。合并后变成一次 batch×2 的调用，两行的 `o_ids` 完全相同，FSDP2 只做一轮 all-gather——省掉一整轮 40 层的参数收发。z 行（行 0）在所有下游使用处都被 `detach`（`cosine_align_loss` 内部会 detach 目标；`inv` 用 `z.detach()`；`bank_nce_loss` 内部 detach `pos_z`），所以反向传播的梯度实际上只流过 z1 行（行 1），z 行的激活存了也不用，多占的显存是 `o_ids` 那几百 token 的量，可忽略。预计省约 10-15% 时间。

两个开关均可独立关掉（`false`）用于排错。`batch_o_z1` 只在 `nce_weight > 0` 时生效（否则没有 z1，无需合并）。如果 CP（context parallel）在 batch=2 的 `o_ids` 前向上行为异常，先关掉这一项。
