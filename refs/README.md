...
## X. 嫁接：切鱼 backbone + 线性 \(W\) 进 Instruct `lm_head`，不是 merge
...
→ **结论：切鱼 + \(W\) 解决的是初始化和对齐门口的问题，不是目标冲突问题。**
目标冲突（WM 的 \(P(o\mid h,a)\) vs agent 的 \(P(a\mid h)\)）仍然要靠 K-draft + JEPA + argmax + token 头架构处理；
切鱼让直筒后半截一开始就会沿残差流写话；\(W\) 让草稿向量能走进那张已经会写命令的表，不用从零训半个十亿参数的 `lm_head`。
预期能减少 Stage 2 需要的数据/步数（省的是这笔钱，不是省掉整个 Stage 2）。
**老实的边界（2026-08-29）：** Layer Swapping 支持直筒仍喂 \(h_{\ell-1}\)。Stage 2 的 `lm_head` 吃的是 \(W(u_i)\)，不是 \(h_{39}\)。切鱼论文没有「重置头后拟合更快」的实验。
