...
Pred(\(z_t,u\)) vs Enc(h,a,o), LDAD from \(\Delta z\), and SIGReg on the
encoder token cloud (mix JSON unwrapped before encode; not observation
token CE, not independent Enc(o) cosine). This branch is named **jepa**, not jepallm.
...
# (CCDF + top-p% mask, not a layer-cut table; writes train/outputs/act/)
...
`/root/tf-logs/{n}_muse_e{epoch}_s{step}/` (`n` = existing run count + 1); use
`python scripts/tmplog.py …` for historical backfill.
